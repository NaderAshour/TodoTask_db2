  ...
  Future opendatabase()async{
    ...
    String path= await join(databasesPath,'db_todo');
     db=await openDatabase(path,version: 1, onCreate:(Database db,int v)async{
       db.execute('create table $todoTable( $columnId integer primary key autoincrement,$columnName text not null,$columnDate integer not null,$columnIsChecked integer not null)');
     });
  }
  Future<TodoDetails> addtodo(TodoDetails todo)async{
    await db.insert(todoTable, todo.toMap(),nullColumnHack:columnId );
    return todo;
  }
  Future<List<TodoDetails>>allTodos()async{
    List<Map<String,dynamic>> todoasmaps=await db.query(todoTable);
     if(todoasmaps.length==0)return [];
     else{
